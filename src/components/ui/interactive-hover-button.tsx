...
const InteractiveHoverButton = React.forwardRef<HTMLButtonElement, InteractiveHoverButtonProps>(({ text = "Show Case", className, ...props }, ref) => {
  return (
    <button
      ref={ref}
      className={cn("group relative md:h-14 h-12 md:w-72 w-40 cursor-pointer overflow-hidden rounded-full border border-primary hover:border-background bg-background p-2 text-center font-semibold md:text-lg text-sm", className)}
      {...props}
    >
